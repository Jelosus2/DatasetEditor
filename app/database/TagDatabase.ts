...
export class TagDatabase {
    ...
    constructor() {
        this.database = this.start();
    }

    start(): DatabaseType {
        if (!fs.existsSync(App.paths.tagAutocompletionsPath))
            App.paths.mkdirRecursive(App.paths.tagAutocompletionsPath);
    ...
    async tryLoadDefaultCsv() {
        ...
        if (rowCount === 0 && fs.existsSync(App.paths.tagAutocompletionFilePath)) {
